...
at first appear:
  Given:
    ...
    w = z,
  Prove: z = w
  Using the axiom of symetry we can assume that since w = z, then z = w.
-/
...
Assume you have an object of a type 'x'.
If you can prove that an arbitrary object of type 'x' has a property of 'P'...
Then you can assume that all objects of type 'x' have a property of type 'P'.
QED.
