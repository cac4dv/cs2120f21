...
def cong_mod_nat (n a b : ℕ) := a % n = b % n
...
begin
  unfold equivalence,
  -- begin 3 stage proof
  /- begin stage 1: reflexivity
    Assume an arbitary value, x, and show that x % n = x % n.
    Can do that via introduction rule of equality.
    QED. -/
  split,
  unfold reflexive,
  unfold cong_mod_nat,
  assume x,
  apply eq.refl,
  /- begin stage 2: symmetry
    Assume abitrary values, x and y, and show that x % n = y % n → y % n = x % n
    Can be proven by ointrroduction rule for equality.
    GED. -/
  split,
  unfold symmetric,
  assume x y,
  apply eq.symm,
  -- begin stage 3: transitivity
  /-
    Assume arbitrary values x, y, and z..
    Then prove that x % n = y % n → y % n → z % n → x % n = z % n
    Can be done via the introduction rule of equaility.
    GE.
  -/
  unfold transitive,
  unfold cong_mod_nat,
...
>>>>>>> a5bff11855f89d7886873103c01d3c9b3be421cc
